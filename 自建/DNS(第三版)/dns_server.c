...
char server_ip[16] = "10.3.9.4";        //默认外部服务器ip地址
...
int main(int argc, char* argv[])
{
    ...
    insert_to_inter();               //读取配置文件dnsrelay.txt，存入内存的url-ip映射表中
    ...
    while (TRUE)                                            //无限循环实现服务器持续监听
    {
        ...
        {
            if (tmp_sockaddr.sin_port == htons(53))         //若该数据报的源地址端口号为53可以确定这是来自外部DNS服务器的数据(这是规定)
            {
