...
void handle_client_message(char *buf,int length, struct sockaddr_in client_addr)
{
    ...
    if (buf + 12 + point != NULL)           //若QTYPE字段非空
    {
        ...
        if (type == 28&& strcmp(ip, "0.0.0.0")!=0)
           {
            ...
            unsigned short new_id = id_transform(ID, client_addr, 0);   //将原id存入id映射表并获得新的id(为了后续服务器代理功能准备-这个顺序就该放在后面)
    ...
    */
    ...
    if (getResult == 0) //若域名在dns_table、cache中均无法找到，需要向外部dns服务器请求帮助
    {
        if (debug_level==2){
        ...
        unsigned short new_id = id_transform(ID, client_addr, 0); //将原客户端id进行转换得到新本地服务器id
    ...
    {
        if (getResult == 1) //如果是从dns_table中查到
        {
            count_s++;
            if (debug_level==2){
        ...
        {
            count_s++;//为了保持请求和响应同步
            /*cache_url_ip = move_to_head(cache_url_ip, new_url);//将记录移到靠前的位置(LRU算法)*/
        ...
        length = sendto(my_socket, sendbuf, pos, 0, (struct sockaddr*)&client_addr, sizeof(client_addr));//将构造好的报文段发给客户端
...
void handle_server_message(char *buf,int length, struct sockaddr_in server_addr)
{
    ...
    if (debug_level)
    {
        ...
        if(debug_level==2){
        show_message(buf, length);//显示完整响应报文
        }
    }
    ...
    ID_table[cur_id].finished = 1;                               //将id映射表中的状态改了，表明该次向外部服务器发送的请求成功
    struct sockaddr_in client_temp = ID_table[cur_id].client_addr;         //获取old_id映射表中的client_addr字段，也就是本地客户端的地址结构体，后续与它进行通信
    ...
    {
        printf("\n#%d:外部DNS服务器查询<URL:%s>成功，解析结果如下:", count_s,url);            //这个url就是我们需要查询的url
    }
    if (num_response == 0 && debug_level==2)
    {
    ...
    for (int i = 0; i < num_response; ++i)                           //分析所有的ANSWER
    {
        ...
        if (type == 1)//TYPE=1表示TYPE A，TYPE=5表示TYPE CNAME
        {
            ip1 = (unsigned char)*p++;
            ip2 = (unsigned char)*p++;
            ip3 = (unsigned char)*p++;
